...
public class StockInvestorProductServiceImpl extends ServiceImpl<StockInvestorProductMapper,StockInvestorProduct> implements StockInvestorProductService {
    ...
    @Override
    public void spiderStockInvestorProduct(String stockCode ,String stockName) {
        String marketNum = "1";
...
//                for(int i = 1 ; i <= 30 ; i++ ){
        String url = "http://data.eastmoney.com/zlsj/detail.aspx?type=ajax&st=2&sr=-1&p=1&ps=1000&jsObj=UwvjQGfy&stat=0&code=" + stockCode + "&date=2019-09-30&rt=" + radomNum();
        ...
        for (int j = 0; j < jsonArray.size(); j++) {
            ...
            investorProduct.setStockCode(stockCode);
            investorProduct.setStockName(stockName);
            ...
            StockInvestorProduct product = selectOne(new EntityWrapper<StockInvestorProduct>().where("investor_code = {0} and stock_code = {1} and static_time = {2}" , SHCode ,stockCode , investorProduct.getStaticTime()));
            if(product == null){
